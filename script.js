...
async function getAudioStream(videoId) {
  const audioUrl = `http://spotify-clone-amk1.onrender.com/audio?url=https://www.youtube.com/watch?v=${videoId}`;
...
async function handleVideoResponse(videoResponse, query) {
  ...
  const song = {
    ...
    playlist: "Liked Songs",
    singer: "Arijit Singh",
    filePath: `http://spotify-clone-amk1.onrender.com/audio?url=https://www.youtube.com/watch?v=${videoId}`
  };
...
async function handleVideoResponse_Gallery(videoResponse, query) {
  ...
  videoResponse.forEach((video, index) => {
    ...
    tr.addEventListener('click', () => {
      ...
      if (currentAudio.src !== `http://spotify-clone-amk1.onrender.com/audio?url=https://www.youtube.com/watch?v=${videoId}`) {
        currentAudio.src = `http://spotify-clone-amk1.onrender.com/audio?url=https://www.youtube.com/watch?v=${videoId}`;
        currentAudio.load();
        currentAudio.play();
        currentAudio.addEventListener('loadedmetadata', () => {
          const duration = formatTime(currentAudio.duration);
